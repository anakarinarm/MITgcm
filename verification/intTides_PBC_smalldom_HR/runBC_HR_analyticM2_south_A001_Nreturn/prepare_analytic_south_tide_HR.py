#!/usr/bin/env python3
"""Generate HR analytical south M2 forcing with north-only return.

This is the HR analogue of the LR `A001_Nreturn` test:

- south normal M2 velocity with A0=0.01 m/s;
- 30-cell cosine taper near the south corners, physically comparable to the
  10-cell LR taper;
- north uniform compensating return flow;
- west prescribed velocity exactly zero;
- no prescribed SSH.

Transport is balanced using MITgcm grid metrics and hFac from the completed HR
telescopic boundary-test run.
"""
...
A0 = 0.01
TAPER_CELLS = 30
TAG = "M2_analyticSouth_A001_Nreturn_HR660x720_telescopic60"

HERE = Path(__file__).resolve().parent
EXPERIMENT = HERE.parent
INPUT = EXPERIMENT / "input"
GRID_DIR = EXPERIMENT / "runBC_telescopic_sponge_test"
...
def main() -> None:
    ...
    north_return = q_s / float(area_n[wet_n].sum())
    north_amp = np.where(wet_n, north_return, 0.0)
    west_amp = np.zeros(NY)
    ...
    print("return architecture: north only")
    print("A0 [m/s]:", A0)
    ...
    print("west max/rms [m/s]:", float(west_amp.max()), float(np.sqrt(np.mean(west_amp**2))))
