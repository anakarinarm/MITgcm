...
The normal velocity files start from the already-balanced obeta velocity
records, so multiplying by a scalar preserves zero net barotropic transport at
each forcing record.  Tangential velocities are scaled by the same scalar to
avoid imposing full-amplitude boundary shear while weakening the normal flow.
"""
